...
const mascaraPlaca = txt => filtro(txt)
                            .replace(/(.{3})(.)/, "$1-$2");
...
const validarCampos = () => {
    if($placa.value.length == 8){
        ...
        const camposVazios = $campos.filter((campo) => campo.value == false)
                                    .map(campos => campos.classList.add("erroCampo"));
...
const identificarAcao = ( eventoBotao ) => {
     ...
     if (acao == "comp"){
        ...
        if(bancoPreco.length != 0){
            ...
            //adicionarRegistroRelatorio(dados[0]);
            //gravarBDRelatorio(adicionarRegistroRelatorio);
            const registroSaida = {
                nome: dados[0].nome,
                placa: dados[0].placa,
                data: dados[0].data,
